...
export function FlowMap() {
  ...
  const criticalPlaza = (plaza?.severity === "HIGH" || plaza?.severity === "CRITICAL") && s.psim.critical > 0;

  return (
    <section className="panel p-5 h-full flex flex-col">
      <div className="flex items-center justify-between mb-2">
        <SectionLabel>Flow Map</SectionLabel>
        <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.12em] text-ink-muted">
          {divertActive && (
            <span className="chip !text-accent !border-accent/40">
              <span className="h-1 w-1 rounded-full bg-accent" /> Divert B→A active
            </span>
          )}
          {s.overlays.rain && (
            <span className="chip !text-info !border-info/40">
              <span className="h-1 w-1 rounded-full bg-info" /> Rain overlay
            </span>
          )}
          {s.vip.active && (
            <span className="chip !text-vip !border-vip/50">
              <span className="h-1 w-1 rounded-full bg-vip" /> VIP inbound
            </span>
          )}
        </div>
      </div>

      <div className="relative flex-1 rounded-md grid-dots overflow-hidden">
        <svg viewBox="0 0 600 320" className="absolute inset-0 w-full h-full">
          {/* Corridors (background paths) */}
          <path
            d="M 50 160 Q 200 70 300 160"
            fill="none"
          ...
          {/* Divert arc (B→A) when active */}
          {divertActive && (
            <path
              d="M 470 110 Q 320 30 130 140"
              fill="none"
              stroke="#3ec1a6"
              strokeWidth="2"
              strokeDasharray="6 8"
          ...
          {/* Medical corridor overlay when plaza critical */}
          {criticalPlaza && (
            <path
              d="M 130 140 L 260 190 L 300 250"
              fill="none"
              stroke="#ef5e5e"
              strokeWidth="3"
              strokeDasharray="5 7"
          ...
          <ZoneNode
            ...
            label="A"
            sub="Gate A"
            value={gateA?.density ?? 0}
            rising={gateA?.trend === "RISING"}
          />
          {/* Gate B */}
          <ZoneNode
            x={470}
            ...
            label="B"
            sub="Gate B"
            value={gateB?.density ?? 0}
            rising={gateB?.trend === "RISING"}
          />
          {/* Plaza */}
          <ZoneNode
            x={300}
            ...
            label="P"
            sub="Plaza Central"
            value={plaza?.density ?? 0}
            rising={plaza?.trend === "RISING"}
            critical={criticalPlaza}
          />
